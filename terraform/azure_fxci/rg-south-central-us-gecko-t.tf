...
resource "azurerm_network_security_group" "sg-south-central-us-gecko-t-default" {
  name                = "sg-south-central-us-gecko-t-default"
  location            = "South Central US"
  resource_group_name = azurerm_resource_group.rg-south-central-us-gecko-t.name
  tags = merge(local.common_tags,
    map(
...
resource "azurerm_virtual_network" "vn-south-central-us-gecko-t" {
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.sg-south-central-us-gecko-t-default.id
  }
  tags = merge(local.common_tags,
    map(
      "Name", "vn-south-central-us-gecko-t"
