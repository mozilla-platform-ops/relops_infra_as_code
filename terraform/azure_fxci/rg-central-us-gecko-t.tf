...
resource "azurerm_network_security_group" "sg-central-us-gecko-t-default" {
  name                = "sg-central-us-gecko-t-default"
  location            = "Central US"
  resource_group_name = azurerm_resource_group.rg-central-us-gecko-t.name
  tags = merge(local.common_tags,
    map(
...
resource "azurerm_virtual_network" "vn-central-us-gecko-t" {
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.sg-central-us-gecko-t-default.id
  }
  tags = merge(local.common_tags,
    map(
      "Name", "vn-central-us-gecko-t"
