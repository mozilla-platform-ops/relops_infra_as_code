...
resource "azurerm_network_security_group" "sg-north-central-us-gecko-t-default" {
  name                = "sg-north-central-us-gecko-t-defaultssg"
  location            = "North Central US"
  resource_group_name = azurerm_resource_group.rg-north-central-us-gecko-t.name
  tags = merge(local.common_tags,
    map(
      "Name", "sg-north-centra-us-gecko-t-default"
    )
  )
}
resource "azurerm_virtual_network" "vn-north-central-us-gecko-t" {
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.sg-north-central-us-gecko-t-default.id
  }
  tags = merge(local.common_tags,
    map(
      "Name", "vn-north-central-us-gecko-t"
