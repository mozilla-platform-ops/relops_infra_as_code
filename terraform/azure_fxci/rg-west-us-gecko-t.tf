...
resource "azurerm_network_security_group" "sg-west-us-gecko-t-default" {
  name                = "sg-west-us-gecko-t-default"
  location            = "West US"
  resource_group_name = azurerm_resource_group.rg-west-us-gecko-t.name
  tags = merge(local.common_tags,
    map(
...
resource "azurerm_virtual_network" "vn-west-us-gecko-t" {
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.sg-west-us-gecko-t-default.id
  }
  tags = merge(local.common_tags,
    map(
      "Name", "vn-west-us-gecko-t"
