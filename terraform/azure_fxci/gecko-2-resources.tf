resource "azurerm_resource_group" "gecko2" {
  for_each = var.gecko2
...
resource "azurerm_storage_account" "gecko2" {
  for_each                 = var.gecko2
...
resource "azurerm_network_security_group" "gecko2" {
  for_each            = var.gecko2
...
resource "azurerm_virtual_network" "gecko2" {
  for_each            = var.gecko2
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.gecko2[each.key].id
  }
}
