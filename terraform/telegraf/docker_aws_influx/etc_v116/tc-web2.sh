...
for provisioner in $provisioners; do
  ...
  for workerType in $workerTypes; do
    if [ $workerType != "gecko-t-win10-64-gpu" ]; then
      continue
    fi
...
#echo $data > tmpout
      ...
      #echo $comma
      echo "$data" 2>&1 | tee $(dirname ${BASH_SOURCE[0]})/tc-web_data.log
      ...
      echo "$data" 2>&1 | tee $(dirname ${BASH_SOURCE[0]})/tc-web_running.log
