...
function get() {
    local host status copied_status state copied_state runId copied_runId start end
    copied_taskId=$1
    taskId=$2
    host=""
    status=$(
    ...
    state=$(echo "$status" | grep -m1 -o 'state": "[^"]*"' | sed -e 's/.*state": "\([^\"]*\)".*/\1/')
    copied_state=$(echo "$copied_status" | grep -m1 -o 'state": "[^"]*"' | sed -e 's/.*state": "\([^\"]*\)".*/\1/')
    if true; then ##! $search_papertrail || [[ $state != 'completed' ]]; then
        workerId=$(echo "$status" | grep -m1 -o 'workerId": "[^"]*"' | sed -e 's/.*workerId": "\([^\"]*\)".*/\1/')
        runId=$(echo "$status" | grep -m1 runId | sed -e 's/.*runId": \([0-9]*\).*/\1/')
        ...
        copied_runId=$(echo "$copied_status" | grep -m1 runId | sed -e 's/.*runId": \([0-9]*\).*/\1/')
        ...
        if false && $search_papertrail; then
            start=$(echo "$status" | grep -m1 -o 'started": "[^"]*"' | sed -e 's/.*started": "\([^\"]*\)".*/\1/')
            end=$(echo "$status" | grep -m1 -o 'resolved": "[^"]*"' | sed -e 's/.*resolved": "\([^\"]*\)".*/\1/')
